...
`i-gol-web-docs/0-AI-prompt-memo.md` は雑記メモであり、AIへの依頼・補助指示の置き場として扱う。

- 明示的なユーザー指示がない限り、このファイルへ AI は書き込みしない。
- ここに書かれた運用上重要なルールを実装へ反映する場合は、必要に応じて本ファイルや `gol-web/lib/ai/` のプロンプト定義へ同期する。

### AI評価ルール（悪習慣）

- 悪習慣に分類される行動は、弛緩/緊張どちらのコーチング文脈でも肯定・称賛しない。
- 悪習慣が含まれる場合は、褒めずに事実の指摘と改善提案を優先する。
- 部分的な工夫があっても、悪習慣そのものを正当化する語（例: 進歩、素晴らしい、よくできた）は使わない。
- 同一事柄に対して、弛緩と緊張で逆の評価を出さない（片方で問題視した対象をもう片方で称賛しない）。
- 指摘対象と褒め対象は分離する。同じ行動を同時に叱責と称賛の両方で扱わない。
