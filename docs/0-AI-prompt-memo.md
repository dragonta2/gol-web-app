...
フラグを付けるとしたらどっちのルールの方がいい？

①
- [x] YID-ひながた｜タスク名｜GOL化
  **｜説明｜**
  - ｜記載日 人間用｜YYMMDD-W
  - ｜難易度｜やさしい・ふつう・むずかしい
  - ｜報酬｜XG｜身体X｜頭脳X｜精神X｜省略せず0のものは0と数字を記載する
  - ｜期限｜YYMMDD-W
  - [] サブタスク名
  - [] サブタスク名
...
経路Bで、ひとつだけ書き込まなければいけない項目がないマークダウンがあった場合の挙動はどうなるの?

A.それだけスルーして作成される
B.アラートが出てすべて作成中止になる


YID-は経路Aでも経路Bでもカードにも表示させるようにして！！！

DBにも登録して、同じYID-のものを2重登録させないようにして！！

GOL-WEB側でタスク名をリネームした場合もYID-で判定するようにして、タスク名が違ってもYIDが同一のものがあったら、重複追加させないようにして！！
...
このファイルの中って完了ブロックに移動させたり
新しいの追加したけど、前に追加されているものも残ってたりした場合に重複してToDo作成しちゃったりしないかなとかもちょっと思ったんだけど


ここもモーダルを開いているときだけ確認できる項目で構わない。
...
ToDoの内容を編集しますと同一行の右端に表示させて

created_atは進行中に動かした日ではない？？
（作成された日？？）

それだったら、これでいいかもしれない。


Gol-WEBのToDoリストに作成日というものを追加したい

見づらいのでレイアウトをかえよう、こうして
作成日:YYYY/MM/DD-W
期限:YYYY/MM/DD-W

この状態で表示させられるようにしたい。

作成日は自己入力 or 現在日時のどちらかを選べるようにする

過去に作ったもので、作成日が不明のものはすべて、2026/01/01でOK。


機能として、これはこれで一旦とっておいていいんだけど、この中の特定の部分を参照して、作成したい。

・足りない情報
・マークダウンファイル側の書式をあわせる

これらを踏まえてまた実装方法を考えて教えて！！


【やりたいこと相談】

■ やりたいこと
27行目以降のチェックリストを作成したら
GOL-WEBのToDoリストのアクティブタスクに
反映されるようになる機能

- マークダウンの書き方の書式はAIの指示によってそろえる

- 「作成する」のようなボタンを押すとGOL-WEB
の方に同期されてアクティブタスクのカードができあがる。

これを実現したいんだけど、どうやればいいかのプランニングをして
...
ToDoリストの未確定スコアは日誌を移動しても、そのまま未確定として別の日誌の画面でも表示させておきたい。
